...
- Scenario 3: PASS for HTTP load and snapshot data. `data.js` contains three recommendations for the Zhongli Station preset destination.
- Scenario 3 update: PASS for local destination switching after regenerating `data.js`; the snapshot now includes full normalized parking lots and availability for client-side re-ranking.
